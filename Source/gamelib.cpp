...
CMovingBitmap AdjustableCInteger::digit[10];
...
void CInteger::LoadBitmap()
{
	...
	if (!isBmpLoaded)
	{
		int d[11] = {IDB_NUM_0, IDB_NUM_1, IDB_NUM_2, IDB_NUM_3, IDB_NUM_4, IDB_NUM_5, IDB_NUM_6, IDB_NUM_7, IDB_NUM_8, IDB_NUM_9, IDB_MINUS};
...
AdjustableCInteger::AdjustableCInteger(int digits)
	: NUMDIGITS(digits)
{
	isBmpLoaded = false;
}

void AdjustableCInteger::Add(int x)
{
	n += x;
}

int AdjustableCInteger::GetInteger()
{
	return n;
}

void AdjustableCInteger::LoadBitmap()
{
	//
	// digit[i]為class varibale，所以必須避免重複LoadBitmap
	//
	if (!isBmpLoaded)
	{
		int d[10] = { IDB_MONEY_NUMBER_0, IDB_MONEY_NUMBER_1, IDB_MONEY_NUMBER_2, IDB_MONEY_NUMBER_3, IDB_MONEY_NUMBER_4, IDB_MONEY_NUMBER_5, IDB_MONEY_NUMBER_6, IDB_MONEY_NUMBER_7, IDB_MONEY_NUMBER_8, IDB_MONEY_NUMBER_9 };
...
void AdjustableCInteger::SetInteger(int i)
{
	n = i;
	double digits = n;
	int times = 0;
...
void AdjustableCInteger::SetTopLeft(int nx, int ny)		// 將動畫的左上角座標移至 (x,y)
{
	x = nx;
	y = ny;
}

void AdjustableCInteger::ShowBitmap()
{
	GAME_ASSERT(isBmpLoaded, "CInteger: 請先執行LoadBitmap，然後才能ShowBitmap");
	int nx;		// 待顯示位數的 x 座標
	int MSB;	// 最左邊(含符號)的位數的數值
